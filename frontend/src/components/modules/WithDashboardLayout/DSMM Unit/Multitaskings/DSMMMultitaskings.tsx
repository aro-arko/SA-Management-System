...
const DSMMMultitaskings = () => {
  ...
  const fetchTasks = useCallback(async () => {
    try {
      ...
      const res = await getDSMMMultitaskings(query);
      setTasks(res.success ? res.data : []);
    } catch (err) {
      console.error("Failed to fetch DSMM multitaskings", err);
    } finally {
      setLoading(false);
    }
    ...
    >
      <div className="max-w-full mx-auto space-y-6">
        <div className="text-center mb-6">
          ...
          <p className="mt-1">Browse all multitasking assignments</p>
...
export default DSMMMultitaskings;
