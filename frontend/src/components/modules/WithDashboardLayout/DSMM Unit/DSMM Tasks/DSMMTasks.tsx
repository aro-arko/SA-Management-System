...
const DSMMTasks = () => {
  ...
  const fetchTasks = useCallback(async () => {
    try {
      ...
      const res = await getDSMMTasks(query);
      if (res.success) {
        setTasks(res.data);
      } else {
        setTasks([]);
      ...
      console.error("Failed to fetch DSMM tasks", error);
    } finally {
      setLoading(false);
    }
    ...
    >
      <div className="max-w-full mx-auto space-y-6">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold tracking-tight">DSMM Tasks</h1>
          <p className="mt-1">Browse all DSMM assignments</p>
