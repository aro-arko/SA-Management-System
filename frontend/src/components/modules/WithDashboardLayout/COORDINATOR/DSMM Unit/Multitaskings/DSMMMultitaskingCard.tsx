...
import { Calendar, ClipboardList, CircleDot } from "lucide-react";
import clsx from "clsx";
import { formatToMalaysiaTime } from "@/utils/formatDate";
import { TDSMMMultitasking } from "@/types/dsmm/multitasking.type";

const DSMMMultitaskingCard = ({ task }: { task: TDSMMMultitasking }) => {
  const { title, taskDate, status } = task;
    ...
    <Card className="w-full border rounded-lg bg-white/80 dark:bg-black/30 shadow-sm hover:shadow-md transition-shadow mb-2">
      <CardContent className="grid grid-cols-3 items-center px-4 py-1 text-[15px] text-muted-foreground h-12">
        ...
        <div className="flex items-center gap-2 justify-self-end">
          ...
          <span
            className={clsx(
              "text-sm font-medium capitalize shrink-0",
              statusColor
            )}
          >
            {status}
...
export default DSMMMultitaskingCard;
