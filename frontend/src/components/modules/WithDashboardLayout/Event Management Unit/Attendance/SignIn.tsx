...
export default function SignIn() {
  // Route: /sign-in/[taskId]/[attendanceId]
  ...
  const handleSubmit = async (e?: any) => {
    ...
    if (!taskId || !attendanceId) {
      toast.error("Invalid or missing sign-in link.");
      return;
    }
    ...
    try {
      setSubmitting(true);
      // Backend expects /sign-in/:attendanceId/:eventId
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_API}/signin-data/sign-in/${taskId}/${attendanceId}`,
        {
          method: "PATCH",
      ...
      if (res.ok && (json?.success ?? true)) {
        ...
        toast.error(json?.message || "Failed to sign in.");
      }
    } catch (e: any) {
  ...
  return (
    <div
      className={clsx(
        ...
        // No JS theme branching — rely on Tailwind dark: classes
    ...
    >
      ...
      >
        <div className="text-center space-y-1">
          <h1 className="text-2xl font-semibold">Event Attendance Sign-In</h1>
        ...
        {linkInvalid && (
          ...
          >
            Warning: This sign-in link looks invalid. You can still fill the
